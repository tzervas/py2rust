...
pub fn emit_function(func: &ast::StmtFunctionDef, source: &str) -> Emitted {
    ...
    let mut args_out: Vec<(String, String, String)> = Vec::new();
        ...
        .chain(func.args.kwonlyargs.iter())
    {
        ...
        // Format filled in after we know which params are loop-rebound (mut).
        args_out.push((aname, rs_aname, ty));
    }
    ...
    let args_fmt: Vec<String> = args_out
        .into_iter()
        .map(|(py_name, rs_aname, ty)| {
            if mut_names.contains(&py_name) {
                format!("mut {rs_aname}: {ty}")
            } else {
                format!("{rs_aname}: {ty}")
            }
        })
    ...
    let sig = if ret_is_unit {
        format!("fn {rs_name}({}) {{", args_fmt.join(", "))
    } else {
        format!("fn {rs_name}({}) -> {ret_ty} {{", args_fmt.join(", "))
    };
...
/// Names that must be declared `let mut` (or `mut` params) because a for/while
/// later reassigns them. Walks in order: a name is "bound" after Assign/AnnAssign
/// (params start bound); assign targets inside a loop that are already bound
/// need mut so the rebind is a true assignment across iterations.
fn names_needing_mut(body: &[ast::Stmt], env: &TypeEnv) -> HashSet<String> {
    let mut bound: HashSet<String> = env.keys().cloned().collect();
    let mut needs = HashSet::new();
    scan_mut_names(body, &mut bound, &mut needs);
    needs
}

fn scan_mut_names(body: &[ast::Stmt], bound: &mut HashSet<String>, needs: &mut HashSet<String>) {
    for stmt in body {
        match stmt {
            ast::Stmt::Assign(a) => {
                if a.targets.len() == 1 {
            ...
            ast::Stmt::For(f) => {
                mark_loop_rebinds(&f.body, bound, needs);
                // Nested accumulators bound inside this loop, rebound in inner loops.
                let mut loop_bound = bound.clone();
                scan_mut_names(&f.body, &mut loop_bound, needs);
            }
            ast::Stmt::While(w) => {
                mark_loop_rebinds(&w.body, bound, needs);
                let mut loop_bound = bound.clone();
                scan_mut_names(&w.body, &mut loop_bound, needs);
            }
            ast::Stmt::If(i) => {
                // Function scope: both arms can bind names visible to later loops.
                scan_mut_names(&i.body, bound, needs);
                scan_mut_names(&i.orelse, bound, needs);
            }
            _ => {}
        }
    }
...
fn mark_loop_rebinds(body: &[ast::Stmt], bound: &HashSet<String>, needs: &mut HashSet<String>) {
...
/// Assign / AnnAssign / AugAssign name targets in `body`, including nested if/for/while.
fn collect_assign_targets(body: &[ast::Stmt], out: &mut HashSet<String>) {
    for stmt in body {
        match stmt {
            ast::Stmt::Assign(a) => {
                if a.targets.len() == 1 {
            ...
            ast::Stmt::If(i) => {
                collect_assign_targets(&i.body, out);
                collect_assign_targets(&i.orelse, out);
            }
            ast::Stmt::For(f) => collect_assign_targets(&f.body, out),
            ast::Stmt::While(w) => collect_assign_targets(&w.body, out),
            _ => {}
        }
    }
}
...
fn try_lower_body(
    ...
    // loop emits a bare assignment (`x = …`) rather than a shadowing `let`,
    // which would be wrong across iterations. The initial binding uses
    // `let mut` when [`names_needing_mut`] marked the name.
    ...
    let mut_names = names_needing_mut(body, env);
        ...
        match stmt {
            ...
            ast::Stmt::Assign(a) => {
                ...
                if rebind_forbidden.is_some_and(|e| e.contains_key(&name)) {
                    lines.push(format!("    {rs_name} = {rhs};"));
                } else if mut_names.contains(&name) && !local_env.contains_key(&name) {
                    // First outer binding of a loop accumulator.
                    local_env.insert(name, want.unwrap_or_else(|| "_".into()));
                    lines.push(format!("    let mut {rs_name} = {rhs};"));
                } else if mut_names.contains(&name) && local_env.contains_key(&name) {
                    // Further outer rebind of a mut accumulator before/between loops.
                    lines.push(format!("    {rs_name} = {rhs};"));
                } else {
            ...
            ast::Stmt::AugAssign(a) => {
                ...
                // Loop rebind (or mut accumulator): real assignment. Otherwise
                // rewrite `x += rhs` as shadow `let x = (x + rhs)` (legal sequential).
                if rebind_forbidden.is_some_and(|e| e.contains_key(&name))
                    || mut_names.contains(&name)
                {
                    lines.push(format!("    {rs_name} = ({rs_name} {bin} {rhs});"));
                } else {
...
fn lower_while(
    ...
    // Env at loop entry is rebind_forbidden: reassignment emits bare assignment.
...
/// Rebinding names bound before the loop uses `let mut` + assignment (not shadow `let`).
fn lower_for(
    f: &ast::StmtFor,
    env: &TypeEnv,
    ret_ty: Option<&str>,
    ...
    let _ = outer_rebind;
    // Env at loop entry: reassignment of these names is bare assignment (mut on outer).
...
fn lower_simple_expr_in(expr: &ast::Expr, env: &TypeEnv, expected: Option<&str>) -> Option<String> {
    match expr {
        ...
        ast::Expr::SetComp(sc) => lower_set_comp(sc, env),
        ast::Expr::DictComp(dc) => lower_dict_comp(dc, env),
        _ => None,
    }
}
...
/// Shape gate shared by emit + walk for set/dict comprehensions: same shape
/// as `is_simple_list_comp_shape` (one generator, `Name` target, no async),
/// just generic over the `generators` field common to all three comp kinds.
fn is_simple_comp_generators_shape(generators: &[ast::Comprehension]) -> bool {
    if generators.len() != 1 {
        return false;
    }
    let g = &generators[0];
...
fn is_simple_set_comp_shape(sc: &ast::ExprSetComp) -> bool {
    is_simple_comp_generators_shape(&sc.generators)
}

fn is_simple_dict_comp_shape(dc: &ast::ExprDictComp) -> bool {
    is_simple_comp_generators_shape(&dc.generators)
}

/// Simple set comprehensions only: one generator, `Name` target, no async.
///
/// `{elt for x in xs}` → `xs.into_iter().map(|x| elt).collect::<HashSet<_>>()`
/// `{x for x in xs if p}` → filter then map/collect.
/// Nested generators, unpack targets, and multiple `if`s stay unlowered.
fn lower_set_comp(sc: &ast::ExprSetComp, env: &TypeEnv) -> Option<String> {
    if !is_simple_set_comp_shape(sc) {
        return None;
    }
    let g = &sc.generators[0];
    let target = match &g.target {
        ast::Expr::Name(n) => n.id.to_string(),
        _ => return None,
    };
    ...
    // Identity `{x for x in xs}` skips map.
    let is_identity = matches!(sc.elt.as_ref(), ast::Expr::Name(n) if n.id.as_str() == target);
    if !is_identity {
        let elt = lower_simple_expr_in(sc.elt.as_ref(), &local, None)?;
        chain.push_str(&format!(".map(|{rs_t}| {elt})"));
    }
    chain.push_str(".collect::<std::collections::HashSet<_>>()");
    Some(chain)
}

/// Simple dict comprehensions only: one generator, `Name` target, no async.
///
/// `{k: v for x in xs}` → `xs.into_iter().map(|x| (k, v)).collect::<HashMap<_,_>>()`
/// `{k: v for x in xs if p}` → filter then map/collect.
/// Nested generators, unpack targets, and multiple `if`s stay unlowered.
fn lower_dict_comp(dc: &ast::ExprDictComp, env: &TypeEnv) -> Option<String> {
    if !is_simple_dict_comp_shape(dc) {
        return None;
    }
    let g = &dc.generators[0];
    let target = match &g.target {
        ast::Expr::Name(n) => n.id.to_string(),
        _ => return None,
    };
    let (rs_t, _) = rust_ident(&target);
    let iter = lower_simple_expr_in(&g.iter, env, None)?;
    // Bind the loop var so names inside key/value/ifs lower (type unknown — rustc decides).
    let mut local = env.clone();
    local.insert(target.clone(), "/* dyn */".into());
    ...
    let key = lower_simple_expr_in(dc.key.as_ref(), &local, None)?;
    let value = lower_simple_expr_in(dc.value.as_ref(), &local, None)?;
    chain.push_str(&format!(".map(|{rs_t}| ({key}, {value}))"));
    chain.push_str(".collect::<std::collections::HashMap<_, _>>()");
    Some(chain)
}
...
fn walk_expr(expr: &ast::Expr, fname: &str, out: &mut Vec<GapReason>) {
    ...
    match expr {
        ...
        ast::Expr::SetComp(sc) => {
            // As with ListComp: only gap shapes `lower_set_comp` does not lower.
            if !is_simple_set_comp_shape(sc) {
        ...
        ast::Expr::DictComp(dc) => {
            // As with ListComp: only gap shapes `lower_dict_comp` does not lower.
            if !is_simple_dict_comp_shape(dc) {
